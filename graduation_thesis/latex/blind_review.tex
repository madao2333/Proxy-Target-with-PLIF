...
\hypersetup{
  ...
  colorlinks=false,
  pdfborder={0 0 0},
  pdftitle={基于脉冲神经网络的连续控制强化学习算法研究},
  pdfauthor={}
}
...
{\zihao{3}\songti\bfseries 姓\quad 名} & \underline{\makebox[70mm][c]{}}\\[10pt]
{\zihao{3}\songti\bfseries 学\quad 号} & \underline{\makebox[70mm][c]{}}\\[10pt]
{\zihao{3}\songti\bfseries 指导教师} & \underline{\makebox[70mm][c]{}}\\
\end{tabular}
\vfill
\end{titlepage}
...
\section{研究背景}
强化学习研究智能体如何在与环境的交互中根据奖励信号学习决策策略。对于机器人控制、仿真运动控制和具身智能等任务，状态和动作通常是连续的，高维非线性动力学使传统表格型强化学习难以直接适用\cite{kober2013rlrobotics}。深度神经网络作为函数逼近器被引入后，Actor-Critic、DDPG、TD3 和 SAC 等算法能够在 MuJoCo 等连续控制环境中学习从状态到动作的端到端策略，其中 actor 负责生成动作，critic 负责估计状态--动作价值。TD3 通过双 critic、目标动作平滑和延迟策略更新缓解函数逼近误差，在连续控制任务中具有较强的稳定性和代表性\cite{mnih2015dqn,lillicrap2016ddpg,fujimoto2018td3,haarnoja2018sac}。

与此同时，脉冲神经网络（Spiking Neural Network，SNN）因其事件驱动计算方式受到关注。与人工神经网络（Artificial Neural Network，ANN）在每层传播连续激活不同，SNN 中神经元通常以膜电位积累和阈值触发的形式产生二值脉冲。只有在脉冲事件发生时才进行有效通信，使 SNN 在神经形态硬件上具有低功耗和低延迟潜力。对于需要长期部署在边缘设备、移动机器人或低功耗控制平台上的强化学习智能体，若能将 actor 网络替换为 SNN，就有机会在保持控制性能的同时降低推理能耗\cite{maass1997third,furber2014spinnaker,davies2018loihi,roy2019neuromorphic,tang2020popsan,chen2024fully}。
...
第二，梳理理论基础。本文将介绍 MDP、Actor-Critic 和 TD3 的基本形式，说明双 critic、延迟策略更新和目标动作平滑在连续控制中的作用；随后介绍 SNN、LIF、PLIF、代理梯度和 STBP，为后续分析梯度消失、放电率饱和和时间常数演化提供概念工具。
...
第四，诊断脉冲 actor 的学习停滞机制。本文以 Hopper-v4 中的默认失败组和 actor 更新间隔调整组为代表案例，结合评估回报、学习曲线、STBP 梯度、放电率、参数梯度和 PLIF 时间常数，分析低回报停滞是否来自脉冲沉默，还是来自高放电状态下的有效梯度消失。
...
在连续控制任务中，动作空间 $\mathcal{A}$ 通常为连续向量空间，策略需要输出实值动作。确定性策略梯度方法使用参数化 actor $\mu_{\phi}(s)$ 直接产生动作，并使用 critic $Q_{\theta}(s,a)$ 估计状态--动作价值\cite{silver2014dpg,lillicrap2016ddpg}。Actor-Critic 框架的核心思想是用 critic 为 actor 提供可微的优化方向，即通过提升 $Q_{\theta}(s,\mu_{\phi}(s))$ 来更新 actor 参数：
\begin{equation}
...
\section{脉冲神经网络}
SNN 用脉冲序列而非连续激活表示和传递信息，具有事件驱动和稀疏计算特征，是神经形态计算中常用的网络模型\cite{maass1997third,davies2018loihi,roy2019neuromorphic,neftci2019surrogate}。一个典型脉冲神经元维护膜电位 $v_t$，接收突触输入电流 $I_t$，当膜电位超过阈值 $V_{\mathrm{th}}$ 时产生脉冲 $s_t$。阈值触发过程可写为
\begin{equation}
s_t=H(v_t-V_{\mathrm{th}}),
\end{equation}
其中 $H(\cdot)$ 为 Heaviside 阶跃函数。由于 $H$ 几乎处处导数为零且在阈值处不可导，SNN 的训练通常需要代理梯度、时间展开反向传播或 ANN-to-SNN 转换等近似方法\cite{bohte2002error,rueckauer2017conversion,lee2016backprop,wu2018stbp,neftci2019surrogate,tavanaei2019deep,eshraghian2023training}。

LIF 神经元在 Integrate-and-Fire 模型基础上加入泄露项，使膜电位随时间衰减，是深层 SNN 和脉冲强化学习 actor 中常用的基础神经元形式\cite{gerstner2002spiking,izhikevich2003simple,lee2016backprop,tang2020popsan,chen2024fully}。本文当前代码中的 LIF 形式可概括为
\begin{equation}
I_t = W x_t + b,
\end{equation}
\begin{equation}
...
\end{equation}
其中 $\Delta$ 为代理梯度窗口宽度，本文实验设为 $0.5$。这意味着只有膜电位落在阈值附近一定范围内时，反向传播才认为脉冲函数具有有效梯度。早期直接训练深层 SNN 的研究已经证明，可以通过时间展开和代理梯度近似将反向传播用于脉冲网络\cite{bohte2002error,lee2016backprop,zenke2018superspike}；STBP 进一步强调误差需要同时沿网络层维度和时间维度传播\cite{wu2018stbp}，SLAYER 等方法也从 spike response 的时间信用分配角度给出了相近思路\cite{shrestha2018slayer}。Neftci 等人的综述以及后续训练方法总结将这类方法归纳为 surrogate gradient learning，为本文分析代理梯度窗口、梯度消失和训练稳定性提供了更一般的背景\cite{neftci2019surrogate,eshraghian2023training}。

STBP 将反向传播同时展开到网络层维度和时间维度\cite{wu2018stbp}。对于第 $l$ 层第 $t$ 个时刻，损失对权重的梯度不仅依赖当前时刻的突触输入，也依赖膜电位沿时间递推产生的历史影响。若膜电位位于阈值附近，代理梯度能够为反向传播提供非零近似；若膜电位长期远离阈值附近区域，或上一时刻频繁放电使递推项中的 $(1-s_{t-1})$ 经常接近零，则时间维度上的梯度传递会被削弱。因此，代理梯度窗口和膜电位递推路径是后续分析 SNN actor 训练状态的重要理论依据。具体诊断指标和实验现象将在后续实验章节展开。
...
由此可见，本文并不是将 PLIF 作为孤立神经元进行讨论，而是将其放入完整的连续控制强化学习闭环中分析。PLIF 提供了可学习的时间尺度自由度，但该自由度需要依赖 actor 更新阶段的 STBP 反向传播才能发挥作用。下一章将进一步说明实验环境、对比方法、性能指标和训练超参数，并在此基础上分析总体性能、学习停滞机制、PLIF 时间常数演化以及 LIF 与 PLIF 的机制近似等价性。

\chapter{实验设计、结果与分析}

\section{实验设置与比较对象}
本文实验在 Gymnasium MuJoCo 连续控制环境上展开。Gymnasium 为强化学习环境提供统一接口，MuJoCo 物理引擎提供了适合机器人运动控制研究的接触动力学仿真平台\cite{towers2024gymnasium,todorov2012mujoco}。选择这些环境的原因在于，它们具有连续状态空间和连续动作空间，能够检验脉冲 actor 在连续控制任务中的动作生成能力，也便于为后续机制分析提供不同难度任务上的结果背景。本文使用的五个具体环境见表 \ref{tab:envs}。
...
表 \ref{tab:hyperparams} 汇总了默认训练和评估设置。这些超参数用于说明实验日志的基本训练条件，并为本章后续关于 \texttt{policy\_freq} 调整、PLIF 时间常数学习率以及 proxy target 更新的讨论提供背景。需要注意的是，表中 \texttt{tau}=0.005 表示 TD3 目标网络软更新系数，而不是 PLIF 神经元中的膜电位保留系数。
...
基于上述实验设置，本章随后先从五个 MuJoCo 环境的最大评估回报和学习曲线分析不同 actor 的总体表现，再以 Hopper-v4 中的代表性训练现象为入口展开机制诊断。
...
从总体结果展示方式上看，最大评估回报能够直接反映不同方法在一次训练过程中达到过的最好策略水平，但它不能单独解释策略如何达到该水平，也不能说明失败组在训练中是否经历过梯度消失。因此，本文还需要结合学习曲线和 STBP 诊断指标进行后续分析。
...
\section{STBP 梯度消失与放电状态诊断}
上一节已经说明，在 Hopper-v4 随机种子 10991 上，默认 PLIF 配置会稳定停留在低回报区间，而将 \texttt{policy\_freq} 从 $2$ 调整为 $4$ 后能够进入高回报区间。本节进一步从 STBP 梯度传播和放电状态两个角度解释这种差异。为保证两组实验之间具有一致的比较基础，本文重点考察梯度绝对值、梯度范数和放电率等诊断指标，并结合进一步记录的代理梯度窗口占比判断空间反向传播链条是否有效。具体聚合时，先在每个训练记录点和每一层内部对 $5$ 个脉冲时间步取平均，再对同组 $5$ 次重复运行取平均。

为了说明“仍然放电”与“仍然可学习”之间的差异，可以从代理梯度在空间反向传播链条中的作用观察梯度消失的来源。对于第 $l$ 层第 $t$ 个时间步，
\begin{equation}
...
I_t^l=W^l s_t^{l-1}+b^l.
\end{equation*}
仅考察同一时间步沿网络层维度传播的梯度，有
\begin{equation}
\frac{\partial \mathcal{L}}{\partial I_t^l}
=
\frac{\partial \mathcal{L}}{\partial s_t^l}
\frac{\partial s_t^l}{\partial v_t^l}
\frac{\partial v_t^l}{\partial I_t^l}
\approx
\frac{\partial \mathcal{L}}{\partial s_t^l}\gamma_t^l,
\label{eq:current-gradient-chain}
\end{equation}
其中 $\partial v_t^l/\partial I_t^l=1$。权重梯度以及向前一网络层继续传递的梯度分别为
\begin{equation}
\begin{aligned}
...
\frac{\partial \mathcal{L}}{\partial I_t^l}
(s_t^{l-1})^\top, \\
\frac{\partial \mathcal{L}}{\partial s_t^{l-1}}
&=
(W^l)^\top
\frac{\partial \mathcal{L}}{\partial I_t^l}.
\end{aligned}
\label{eq:spatial-gradient-chain}
\end{equation}
其中，$s_t^{l-1}$ 表示同一时间步来自前一网络层的脉冲。由上述链条可见，当某一层的膜电位全部落在代理梯度窗口外时，$\gamma_t^l=0$，该层的电流梯度随之变为零；其普通权重无法继续获得有效更新，向前一网络层传播的空间梯度也会被截断。

本文 SNN actor 的连续动作由动作输出层脉冲在时间维度上累积后经种群解码器生成，不存在绕过输出脉冲函数的膜电位读出路径。因此，当动作输出层的代理梯度窗口占比为零时，策略损失难以穿过动作输出脉冲传入 SNN 内部，动作输出层会形成关键的梯度瓶颈。膜电位沿时间维度的递推仍然存在，但在缺少输出端误差信号时不能产生新的梯度。此时，即使前向传播仍产生脉冲活动，前序隐藏层权重、偏置以及 PLIF 时间常数参数也难以继续从策略损失获得有效更新。后续诊断将围绕这一反向传播链条展开。

为便于理解后续诊断图，表 \ref{tab:stbp-diagnostic-metrics} 汇总了本节使用的主要指标。整体上，代理梯度窗口占比用于判断脉冲函数近似导数是否仍然有效，梯度指标用于判断 actor 是否仍能获得反向学习信号，放电率指标用于判断前向脉冲活动是否沉默或饱和。
...
代理梯度窗口占比 & $\mathbb{E}[\mathbb{I}(|v_t^l-V_{\mathrm{th}}|<\Delta)]$ & 膜电位落入代理梯度有效区域的比例，用于判断脉冲函数近似导数是否能够提供反向信号。 \\
...
图 \ref{fig:plif-stbp-gradient-collapse} 给出了三层 actor 在训练过程中的 STBP 梯度变化。为了同时显示早期小梯度和后期零梯度，纵轴采用对数坐标，精确为零的值在图中用 $10^{-12}$ 显示。从图中可以看到，默认失败组的电流梯度、逐时间步权重梯度、参数级权重梯度以及时间常数梯度都在训练早期后迅速消失。五次重复运行中，最后一次非零梯度均出现在第 $19800$ 次训练迭代，之后直到第 $1000000$ 次训练迭代均不再记录到非零梯度。

需要区分短暂的局部零梯度与持续性的反向传播中断。在 \texttt{policy\_freq}=4 的 actor 更新间隔调整组中，动作输出层的时间步权重梯度和参数级权重梯度在 \texttt{train\_it}$=800$--$1200$ 以及 \texttt{train\_it}$=2400$ 时短暂为零，PLIF 时间常数梯度在 \texttt{train\_it}$=800$--$4800$ 区间内短暂为零；但三层电流梯度在全部记录点均保持非零，后续权重梯度和时间常数梯度也能够恢复。这说明，训练早期单次或短区间内出现局部零梯度并不足以判定训练失败。actor 每次更新使用的 replay buffer 状态批次不同，网络参数也在持续变化，后续更新仍可能使膜电位重新进入代理梯度窗口。

默认失败组则表现出不同的持续状态。进一步记录的代理梯度窗口占比表明，动作输出层在训练早期后持续为零。根据式 \eqref{eq:current-gradient-chain} 和式 \eqref{eq:spatial-gradient-chain}，当动作输出层的 $\gamma_t^l$ 持续为零时，其电流梯度变为零，策略损失无法继续传入 SNN 内部，前序隐藏层的空间梯度也随之被截断。由于 SNN 内部参数难以继续更新，膜电位分布也难以依靠梯度优化重新进入有效窗口，网络由此进入持续性的低回报停滞状态。需要说明的是，现有结果能够解释该状态为何持续维持，但尚不足以严格确定训练早期由哪一次更新首先触发了输出端梯度截断。
...
为了进一步解释图 \ref{fig:plif-stbp-gradient-collapse} 中梯度消失的来源，本文根据前述代理梯度公式统计两组实验的代理梯度窗口占比，如图 \ref{fig:plif-fail-surrogate-window} 和图 \ref{fig:plif-success-surrogate-window} 所示。这两张图的作用是将式 \eqref{eq:current-gradient-chain} 和式 \eqref{eq:spatial-gradient-chain} 中的 $\gamma_t^l\approx\mathbb{I}(|v_t^l-V_{\mathrm{th}}|<\Delta)$ 可视化：若某层窗口占比长期接近零，则该层脉冲函数的近似导数难以继续提供反向学习入口；若窗口占比保持非零，则 STBP 链条仍有可能获得有效代理梯度。

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.50\textheight,keepaspectratio]{../../figures/plif_default_surrogate_window_early.pdf}
\caption{Hopper-v4 默认失败组的代理梯度窗口占比。纵轴为 $\mathbb{E}[\mathbb{I}(|v_t^l-V_{\mathrm{th}}|<\Delta)]$，表示各层膜电位落入代理梯度有效窗口的比例。}
\label{fig:plif-fail-surrogate-window}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.50\textheight,keepaspectratio]{../../figures/plif_success_20260530_204900_surrogate_window.pdf}
\caption{Hopper-v4 actor 更新间隔调整成功组的代理梯度窗口占比。与默认失败组相比，成功组在隐藏层和动作输出层均能长期保留非零代理梯度窗口占比。}
\label{fig:plif-success-surrogate-window}
\end{figure}

综合来看，默认失败组的关键问题不是前向脉冲完全消失，而是动作输出层代理梯度窗口占比持续为零，导致策略损失难以传入 SNN 内部。图 \ref{fig:plif-stbp-spike-rates} 表明失败组后期仍有明显脉冲活动；但根据式 \eqref{eq:current-gradient-chain} 和式 \eqref{eq:spatial-gradient-chain}，能否继续学习还取决于动作输出层是否保留非零代理梯度。图 \ref{fig:plif-fail-surrogate-window} 显示，失败组动作输出层的代理梯度窗口占比在训练早期后长期接近零；图 \ref{fig:plif-stbp-gradient-collapse} 中的电流梯度、权重梯度和 PLIF 时间常数梯度进一步表明，失败组后期已经无法继续获得有效反向学习信号。相比之下，图 \ref{fig:plif-success-surrogate-window} 中成功组三层窗口占比均保持非零水平，说明其反向传播链条仍保留可用的代理梯度入口；配合更稀疏的隐藏层放电和持续非零 STBP 梯度，actor 能够继续通过 critic 提供的策略损失更新参数，最终进入高回报区间。

在当前实现中，\texttt{policy\_freq} 从 $2$ 调整为 $4$ 后，相邻两次 actor 更新之间会执行更多次 critic 与 proxy target 优化。因此，成功组表现出更强训练稳健性的一种可能解释是：critic 对动作价值函数的估计和 proxy target 对在线 actor 的拟合更加充分，使后续策略更新所依赖的优化信号相对稳定，从而降低了训练早期进入持续性梯度失效状态的可能性。需要说明的是，现有对照实验尚不足以区分 critic 拟合、proxy target 拟合和 actor 更新节奏各自的独立贡献，因此该解释应理解为基于实现过程与实验现象的机制推测，而不是严格的单因素因果结论。

需要再次强调，本文中的五次重复运行使用相同随机种子，因此本节结论主要说明相同设置下的机制可复现性，而不是跨随机种子的统计显著性。不过，这一对照已经足以支持本文的核心机制判断：在当前 Proxy Target + PLIF 训练中，失败组的低回报停滞更接近“仍然放电但梯度消失”的状态，而不是简单的脉冲沉默或动作输出缺失。

\FloatBarrier

\section{PLIF 时间常数的层间演化}
上一节说明了成功组的反向传播链条能够从训练早期的局部零梯度中恢复，并在后期持续保留有效 STBP 梯度。本节进一步考察 PLIF 的可学习时间常数在训练中具体学到了什么。图 \ref{fig:plif-tau-curves} 展示了常规 PT-PLIF 实验中不同环境下三层 PLIF 时间常数的平均演化曲线。整体上，隐藏层时间常数往往从初始化附近逐渐升高，而动作输出层时间常数的变化更保守，在 Hopper-v4、HalfCheetah-v4 和 Walker2d-v4 中甚至出现后期下降。这说明 PLIF 并不是在所有层上无差别地增大时间常数，而是呈现出明显的层间分化：隐藏层更倾向于学习较长的膜电位保留时间，动作输出层则更直接服务于动作种群的读出，对时间常数的需求可能更短。
...
在机制分析部分，本文以 Hopper-v4 中的 PLIF 失败组和 actor 更新间隔调整组作为代表案例，进一步考察 STBP 梯度传播、放电状态和 PLIF 时间常数演化之间的关系。分析表明，失败组并不是简单进入脉冲沉默状态，而是在前向传播仍保持较高放电率的同时，反向传播中的有效 STBP 梯度已经消失，导致权重、偏置和 PLIF 时间常数难以继续获得学习信号。相比之下，actor 更新间隔调整组在隐藏层保持更稀疏的放电状态，并持续保留非零梯度，使 actor 能够继续通过 critic 提供的策略损失更新参数。
...
第二，Hopper-v4 默认失败组揭示了一类“前向仍放电、反向已失效”的训练停滞模式。失败组后期并未进入脉冲沉默，隐藏层和动作输出层仍保持较高放电率；但由于电流幅值较大、膜电位分布更分散，同时频繁放电通过重置项削弱膜电位历史路径，STBP 有效梯度逐渐消失。此时 actor 虽然仍能产生动作，但权重、偏置和 PLIF 时间常数都难以继续获得有效学习信号，评估回报也长期停留在低水平。
...
其次，失败机制分析主要围绕 Hopper-v4 的代表性案例展开。该环境能够清楚展示“仍然放电但梯度消失”的现象，也便于对失败组和成功组进行对照；但相同现象在其他环境中的出现概率、触发条件和缓解方式仍需要更多系统实验支撑。本文中的重复运行主要用于说明同一设置下机制现象可以复现，而不是严格意义上的跨随机种子统计显著性检验。
...
Silver D, Lever G, Heess N, et al. Deterministic Policy Gradient Algorithms[C]//Proceedings of the 31st International Conference on Machine Learning. Beijing, China: PMLR, 2014: 387-395.
...
Towers M, Kwiatkowski A, Terry J, et al. Gymnasium: A Standard Interface for Reinforcement Learning Environments[EB/OL]. arXiv:2407.17032, 2024.

\bibitem{maass1997third}
...
Gerstner W, Kistler W M. Spiking Neuron Models: Single Neurons, Populations, Plasticity[M]. Cambridge: Cambridge University Press, 2002.
...
Furber S B, Galluppi F, Temple S, Plana L A. The SpiNNaker Project[J]. Proceedings of the IEEE, 2014, 102(5): 652-665.

\bibitem{roy2019neuromorphic}
Roy K, Jaiswal A, Panda P. Towards Spike-Based Machine Intelligence with Neuromorphic Computing[J]. Nature, 2019, 575(7784): 607-617.
...
Bohte S M, Kok J N, La Poutré H. Error-Backpropagation in Temporally Encoded Networks of Spiking Neurons[J]. Neurocomputing, 2002, 48(1-4): 17-37.

\bibitem{rueckauer2017conversion}
Rueckauer B, Lungu I A, Hu Y, Pfeiffer M, Liu S C. Conversion of Continuous-Valued Deep Networks to Efficient Event-Driven Networks for Image Classification[J]. Frontiers in Neuroscience, 2017, 11: 682.
...
Tavanaei A, Ghodrati M, Kheradpisheh S R, Masquelier T, Maida A. Deep Learning in Spiking Neural Networks[J]. Neural Networks, 2019, 111: 47-63.

\bibitem{eshraghian2023training}
Eshraghian J K, Ward M, Neftci E O, et al. Training Spiking Neural Networks Using Lessons from Deep Learning[J]. Proceedings of the IEEE, 2023, 111(9): 1016-1054.
