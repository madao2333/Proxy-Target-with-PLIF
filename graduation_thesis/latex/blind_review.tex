...
脉冲神经网络具有事件驱动、稀疏计算和低功耗潜力，但其离散脉冲机制使连续控制强化学习面临梯度传播和连续动作输出困难。本文在 Proxy Target + TD3 框架下，比较 LIF 与 PLIF 脉冲 actor 在五个 MuJoCo 环境中的表现，并结合 STBP 梯度、放电状态和 PLIF 时间常数演化分析训练机制。实验表明，PT-PLIF 相比 PT-LIF 未呈现稳定的最终回报优势，PLIF 不应被视为全面优于 LIF 的替代神经元。Hopper-v4 案例显示，低回报停滞与有效梯度塌缩和时间递推路径削弱有关，调整 actor 更新间隔可缓解该现象。成功训练组中，PLIF 时间常数呈层间分化，LIF 与 PLIF 的配对机制指标高度一致，说明二者在当前框架下具有机制近似等价性，PLIF 的主要价值在于提供可解释的时间尺度自适应能力。

\noindent{\songti\zihao{4}\bfseries 关键词：}{\songti 脉冲神经网络，连续控制，强化学习，Proxy Target，PLIF 神经元}

\chapter*{\zihao{2}\bfseries ABSTRACT}
Spiking neural networks offer event-driven, sparse, and low-power computation, but discrete spikes hinder gradient propagation and continuous action output in control reinforcement learning. Under Proxy Target + TD3, this thesis compares LIF and PLIF spiking actors on five MuJoCo tasks and analyzes STBP gradients, firing states, and PLIF time constants. Results show no stable return advantage of PT-PLIF over PT-LIF, so PLIF is not a universally superior replacement. Hopper-v4 diagnostics connect low-return stagnation to gradient collapse and weakened temporal recurrence, while adjusted actor updates alleviate it. Successful runs show layer-wise PLIF time constants and highly consistent paired LIF/PLIF diagnostics, suggesting approximate mechanism equivalence, with PLIF mainly offering interpretable time-scale adaptation.
...
在 SNN 神经元模型中，Leaky Integrate-and-Fire（LIF）神经元以固定衰减系数描述膜电位随时间的泄露，是最常用的基础模型之一。Parametric Leaky Integrate-and-Fire（PLIF）进一步将膜电位时间常数设为可学习参数，使网络不仅学习突触权重，也学习神经元自身的时间尺度\cite{fang2021plif}。从直觉上看，连续控制任务包含不同的时序依赖，不同环境和不同层可能需要不同的膜电位保持能力，因此 PLIF 似乎有可能提高脉冲 actor 的适应能力。但在 Proxy Target + TD3 框架下，本文实验显示 PT-PLIF 与 PT-LIF 的最终回报整体接近，并不存在稳定的全面优势。因此，本文的核心问题不是简单证明 PLIF 比 LIF 更强，而是分析可学习时间常数是否真正改变了训练动力学：一方面解释脉冲 actor 为什么会在某些设置下“仍然放电但学习不动”，另一方面判断 LIF 与 PLIF 在成功训练状态下是否形成了相近的内部机制。

\section{问题提出}
传统 LIF 神经元使用固定的膜电位保留系数。以本文代码中的实现为例，LIF 隐藏层和输出 population 层使用相同的固定系数 $\tau_{\mathrm{LIF}}=0.75$，膜电位按照上一时刻膜电位、上一时刻脉冲重置项和当前输入电流共同更新。固定系数的优点是结构简单、参数量少、可解释性强；缺点是网络各层在训练前就被赋予相同时间尺度，难以根据任务动态自适应地调整记忆长度。

PLIF 将这一固定系数替换为可学习参数。若训练信号充分，隐藏层可以学习更长或更短的膜电位保持时间，输出层也可以根据动作读出需求调整放电节奏。Fang 等人的研究表明，在监督学习任务中，学习膜时间常数能够增强 SNN 的表达能力并降低对初始值的敏感性\cite{fang2021plif}。因此，将 PLIF 引入强化学习中的 SNN actor 是自然的扩展。

但是，强化学习中的训练信号比监督学习更间接、更噪声化。actor 并不是直接拟合静态标签，而是通过 critic 对动作价值的估计间接更新；critic 本身又由 replay buffer、目标网络和时序差分目标共同驱动。对于 SNN actor，策略损失的反向传播还要穿过时间维度上的膜电位、脉冲发放和重置路径。如果早期训练使膜电位长期远离代理梯度有效窗口，则即使前向仍能产生大量脉冲，反向梯度也可能接近零。此时 PLIF 的额外时间常数参数并不会自动发挥作用，因为它同样依赖 STBP 提供学习信号。

因此，本文需要回答的问题并不是 PLIF 是否在最终回报上简单优于 LIF，而是可学习时间常数在 Proxy Target + TD3 训练闭环中是否真正改变了脉冲 actor 的学习过程。具体而言，本文关注三类问题：第一，低回报停滞究竟来自脉冲活动不足，还是来自有效反向梯度路径失效；第二，PLIF 时间常数在成功训练中是否呈现可解释的层间调节；第三，在相近性能条件下，LIF 与 PLIF 是否形成了显著不同的内部训练机制。这些问题构成了后续实验设计和机制分析的出发点。

\section{研究目标}
本文围绕 Proxy Target + TD3 框架下 LIF 与 PLIF 脉冲 actor 的训练机制展开，具体目标如下。

第一，建立问题叙述。本文从连续控制强化学习、SNN actor 的低功耗潜力以及 Proxy Target 框架的必要性出发，引出脉冲 actor 中膜电位时间尺度建模问题。在此基础上，本文将固定膜电位保留系数的 LIF 与可学习时间常数的 PLIF 进行对照，并明确本文不以证明“PLIF 全面优于 LIF”为目标，而是关注二者在当前框架下的性能接近性和训练机制差异。
...
第四，诊断脉冲 actor 的学习停滞机制。本文以 Hopper-v4 中的默认失败组和 actor 更新间隔调整组为代表案例，结合评估回报、学习曲线、STBP 梯度、放电率、电流、膜电位、参数梯度和 PLIF 时间常数，分析低回报停滞是否来自脉冲沉默，还是来自高放电状态下的有效梯度塌缩。
...
由此可见，本文并不是将 PLIF 作为孤立神经元进行讨论，而是将其放入完整的连续控制强化学习闭环中分析。PLIF 提供了可学习的时间尺度自由度，但该自由度需要依赖 actor 更新阶段的 STBP 反向传播才能发挥作用。第四章将进一步说明实验环境、对比方法和评价指标，第五章则在此实现基础上分析总体性能、学习停滞机制、PLIF 时间常数演化以及 LIF 与 PLIF 的机制近似等价性。

\chapter{实验设计与机制分析框架}

\section{MuJoCo 任务选择与评估协议}
...
\section{Actor 对照对象与核心比较关系}
本文实验包含 ANN、LIF、PLIF 和 CLIF 四类 actor 或神经元设置。ANN 是非脉冲 TD3 actor baseline，不包含脉冲发放和膜电位递推，用于提供传统连续激活 actor 的结果参照。CLIF 在脉冲神经元中进一步引入电流动态，用于补充观察额外神经元动力学对训练表现的影响。二者在本文中主要承担背景参照作用，而不是机制分析的核心对象。

本文的核心比较对象是 LIF 与 PLIF。二者共享相同的状态 population 编码器、Spike MLP 结构、动作 population 解码器、critic 结构和 Proxy Target 训练流程，主要差异在于膜电位保留系数是否可学习。因此，若二者最终回报接近，不能简单说明 PLIF 无效，而应进一步观察时间常数是否获得有效梯度、隐藏层放电是否健康、电流和膜电位是否落在适合代理梯度传播的范围内。

基于这一定位，本文不将四类 actor 的比较写成单纯的性能排序。ANN 和 CLIF 用于说明总体结果背景，LIF 与 PLIF 则用于回答本文的核心问题：可学习时间常数是否改变 Proxy Target 脉冲 actor 的训练机制，以及二者在成功训练状态下是否具有近似等价的内部动力学。

\section{性能指标、诊断指标与训练超参数}
本文的实验指标分为性能结果和机制诊断两类。性能结果用于提供整体背景，包括最大评估回报和平均学习曲线。对于每个环境、每个算法和每个随机种子，训练日志中记录一系列评估回报，最大评估回报定义为这些记录中的最大值。随后对同一环境下五个随机种子的最大评估回报计算均值和标准差，用于比较不同方法的整体水平和随机种子间稳定性。平均学习曲线则用于观察收敛速度、早期失败和后期波动。

为了分析训练机制，本文还记录神经动力学、梯度和参数三类诊断指标。神经动力学指标包括阈值前放电率、阈值后放电率、电流均值、电流绝对值均值、膜电位均值和膜电位标准差，用于判断网络处于沉默、健康放电还是饱和放电状态。梯度指标包括电流梯度、权重和偏置梯度、带符号梯度均值和 PLIF 时间常数梯度，用于判断 STBP 是否仍能提供学习信号。参数指标包括权重均值、权重标准差、权重范数和偏置均值，用于观察 actor 是否形成整体正偏、权重分化或参数冻结。
...
\section{结果组织方式与机制分析边界}
为保证实验设置与结果分析之间具有明确对应关系，本节对实验结果的来源、组织方式和分析边界作统一说明。本文整理的训练日志覆盖 ANN、LIF、PLIF 和 CLIF 四类 actor 设置，并在表 \ref{tab:envs} 所列的五个 MuJoCo 连续控制环境上进行比较。各方法的总体表现主要依据前文定义的最大评估回报和平均学习曲线进行统计，用于说明不同 actor 设置在当前实验范围内的整体表现，而不是作为本文唯一结论依据。

在结果组织上，本文先从四类方法的整体性能入手，给出 ANN、LIF、PLIF 和 CLIF 在不同环境中的总体对比。ANN baseline 用于提供传统连续激活 actor 的性能参照，CLIF 用于补充观察引入额外神经元动力学后的训练表现。随后，分析重点回到 LIF 与 PLIF 的比较。由于二者在网络结构、状态编码、动作解码、critic 以及 Proxy Target 训练流程上保持一致，其差异主要来自膜电位保留系数是否可学习，因此仅比较最终回报不足以判断 PLIF 是否发挥作用，还需要结合时间常数演化、放电状态、膜电位分布和梯度传播状态进行解释。

除总体结果展示外，本文将 Hopper-v4 作为机制分析的代表环境。该环境在脉冲 actor 训练中更容易出现低回报停滞现象，适合用于进一步检查训练失败的内部原因。围绕该环境，本文整理了默认 PLIF 失败配置、调整 actor 更新频率后的成功配置以及相关对照实验的梯度与神经状态诊断信息，并结合参数统计结果，从放电率、电流、膜电位、权重梯度、偏置梯度和 PLIF 时间常数梯度等角度进行诊断。该部分分析的目的不是单纯报告某个超参数组合取得更高回报，而是说明低回报停滞与脉冲活动、代理梯度窗口和时间递推路径之间的关系。

因此，第五章的实验结果将按照四个层次展开：首先展示四类方法在多个环境中的总体结果；其次结合学习曲线分析训练过程中的成功、停滞与波动；随后通过 STBP 梯度、神经动力学状态和 PLIF 时间常数演化解释 Hopper-v4 案例中的学习失败与缓解机制；最后比较成功训练状态下 LIF 与 PLIF 的配对机制指标，讨论二者在当前框架下的机制近似等价性。这样的安排能够使总体结果服务于背景说明，使机制诊断服务于本文的核心问题。
...
\]
由于本文代码中每个 PLIF 层共享一个标量时间常数，表 \ref{tab:policy-freq-tau} 中进一步使用同层阈值后平均放电率 $\bar{s}$ 近似该平均量，即 $R_l\approx\tau_l(1-\bar{s})$。
