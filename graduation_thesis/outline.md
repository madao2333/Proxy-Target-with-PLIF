...
- 训练轨迹可视化整理：
  - 将失败组、调参成功组和 LIF 对照组的 trace 统一成可复用的图表格式。
  - 对齐 reward、spike rate、current/volt、参数摘要和梯度曲线的训练步数。
  - 优先补足能够支撑“失败机制”和“LIF/PLIF 接近性”的图，避免引入新的 tau 取值对照实验。
...
- 总结 SNN actor 在连续控制任务中的潜力与训练困难。
- 总结本文基于 Proxy Target + TD3 框架，对 ANN、LIF、PLIF 和 CLIF 等 actor 设置进行实验比较。
- 强调本文没有将 PLIF 表述为全面优于 LIF，而是结合学习曲线、STBP 梯度、放电率、电流、膜电位、PLIF 时间常数和有效保留项分析脉冲 actor 的训练机制。

### 6.2 主要研究结论

- PT-PLIF 与 PT-LIF 在当前实验范围内整体表现接近，不支持“PLIF 稳定全面优于 LIF”的强结论。
- Hopper-v4 默认失败组的关键现象不是脉冲沉默，而是前向仍然放电但有效 STBP 梯度塌缩。
- actor 更新间隔调整组说明，更稀疏的隐藏层放电、更健康的膜电位尺度和持续非零梯度有助于缓解低回报停滞。
- PLIF 时间常数呈现层间分化，隐藏层倾向于增大时间常数并提高有效膜电位保留能力，动作输出层不必同步增大。
- LIF/PLIF 等价性分析后续补充到第五章中，作为机制分析的一部分，而不作为第六章研究不足展开。

### 6.3 研究不足

- 实验覆盖范围仍可扩展，当前环境数量、随机种子数量和超参数组合还不足以穷尽所有训练情形。
- Hopper-v4 机制分析主要围绕代表性案例展开，相同随机种子的重复运行更适合说明机制可复现性，而不是跨随机种子的统计显著性。
- 对 `proxy_lr`、`policy_freq` 和 PLIF 时间常数学习率等超参数之间的交互关系，还没有形成完整网格化分析。
- 本文主要关注训练机制，对脉冲 actor 的脉冲率、能耗、推理延迟和神经形态硬件部署收益尚未进行系统评估。

### 6.4 未来展望

- 扩展更多连续控制环境、随机种子和超参数组合，验证结论的稳健性。
- 系统研究 `proxy_lr`、`policy_freq` 和 PLIF 学习率对训练稳定性的影响。
- 在第五章补充 LIF/PLIF 等价性分析所需诊断指标后，进一步形成更完整的机制对照框架。
- 面向实际部署，进一步评估脉冲率、稀疏性、能耗、推理延迟和神经形态硬件适配。
